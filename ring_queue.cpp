...
class RingQueue{
  ...
  public:
    ...
    class iterator{
      ...
      public:
        reference operator*() {
	  int begin =  parent-> begin_index;
	  /*operator*() should return the array location indexed by (begin_index + offset) % ring_capacity.*/
	  return parent->buffer[(begin + offset) % MAX_SIZE];
        }

        iterator& operator++(){
	  // int ring = parent-> ring_size;
	  //int shift = parent -> begin_index;
	  //offset = ((offset + 1) % ring) + shift; //shift it by index
	  ++offset;
	  return *this;
        }

        iterator operator++( int unused ){
	  iterator temp(this, offset);
	  ++this;
          return *temp;
        }
  ...
  public:
    ...
    ItemType back() const {
        if ( ring_size == 0 ) std::cerr<< "Warning: Empty ring!\n" ;
	int end = end_index();
	//	if (end==0) return buffer[MAX_SIZE-1]; // it must be tht 
	if (end==0) return buffer[ring_size];
	return buffer[end-1];
    }
    ...
    void pop_front(){
      if ( ring_size == 0 ) std::cerr<< "Warning: Empty ring!\n" ;
      begin_index = (begin_index + 1) % ring_size;
    ...
    iterator begin() {
      return iterator(this,0); // since the offset is in relation to the begin_index it must be 0 
    }
